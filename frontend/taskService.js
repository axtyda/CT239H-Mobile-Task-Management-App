...
const getLocalDate = (dateInput) => {
  return dayjs(dateInput || undefined)
    .tz(dayjs.tz.guess())
    .toDate()
}
    ...
    try {
      realm.write(() => {
        ...
        if (task) {
          ...
          // Handle subGoals if provided using differential updates
          if (updatedFields.subGoals) {
            // Get existing subgoal IDs
            const existingSubGoalIds = new Set(
              Array.from(task.subGoals).map(sg => sg.subGoalId)
            );
            
            // Get updated subgoal IDs
            const updatedSubGoalIds = new Set(
              updatedFields.subGoals.map(sg => sg.subGoalId)
            );
            
            // Find subgoals to delete (in existing but not in updated)
            const subGoalsToDelete = Array.from(task.subGoals).filter(sg => 
              !updatedSubGoalIds.has(sg.subGoalId)
            );
            
            // Delete subgoals that are no longer present
            for (const sg of subGoalsToDelete) {
              const index = task.subGoals.findIndex(s => s.subGoalId === sg.subGoalId);
              if (index !== -1) {
                task.subGoals.splice(index, 1);
              }
            }
            
            // Update existing and add new subgoals
            for (const updatedSg of updatedFields.subGoals) {
              if (existingSubGoalIds.has(updatedSg.subGoalId)) {
                // Update existing subgoal
                const existingSg = task.subGoals.find(sg => sg.subGoalId === updatedSg.subGoalId);
                if (existingSg) {
                  existingSg.name = updatedSg.name;
                  existingSg.isCompleted = updatedSg.isCompleted || false;
                }
              } else {
                // Add new subgoal with a guaranteed unique ID
                const newSubGoalId = updatedSg.subGoalId || 
                  `${Date.now()}-${Math.random().toString(36).substring(2, 9)}`;
                
                task.subGoals.push({
                  subGoalId: newSubGoalId,
                  name: updatedSg.name,
                  isCompleted: updatedSg.isCompleted || false,
                });
              }
            }
            
            // Remove subGoals from updatedFields to prevent double processing

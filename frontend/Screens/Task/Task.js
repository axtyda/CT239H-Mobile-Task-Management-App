...
export default function Task() {
  ...
  const handleSearch = (text) => {
    ...
    const results = tasks.filter((task) => task.title.toLowerCase().includes(text.toLowerCase()))
  ...
  const renderCalendarDay = (dayObj) => {
    ...
    return (
      <TouchableOpacity
        ...
        style={[styles.calendarDay, !dayObj.isCurrentMonth && styles.otherMonthDay, isSelected && styles.selectedDay]}
      ...
      >
        <Text
          style={[
            ...
            !dayObj.isCurrentMonth && styles.otherMonthDayText, dynamicStyles.otherMonthDayText,
      ...
      <View style={styles.weekViewContainer}>
        <View style={styles.weekDaysHeader}>
          {weekDays.map((wDay, index) => (
            <Text key={`weekday-${index}`} style={[styles.weekDayText, dynamicStyles.weekDayText]}>
              {wDay}
            </Text>
          ))}
        </View>
        <View style={styles.weekDaysRow}>
          {weekDaysData.map((dayObj, idx) => (
            <React.Fragment key={`weekday-${idx}`}>{renderCalendarDay(dayObj)}</React.Fragment>
          ))}
        </View>
      </View>
    )
  ...
  const renderMonthlyCalendar = () => {
    ...
    return (
      <View style={styles.monthViewContainer}>
        <View style={styles.weekDaysHeader}>
          {weekDays.map((wDay, index) => (
            <Text key={`monthday-${index}`} style={[styles.weekDayText, dynamicStyles.weekDayText]}>
              {wDay}
            </Text>
          ))}
        </View>
          ...
          {weeks.map((week, weekIndex) => (
            <View key={`week-${weekIndex}`} style={styles.weekRow}>
              {week.map((dayObj, dayIndex) => (
                <React.Fragment key={`day-${weekIndex}-${dayIndex}`}>{renderCalendarDay(dayObj)}</React.Fragment>
              ))}
            </View>
          ))}
        </View>
        ...
        <View style={styles.taskContent}>
          ...
          <View style={styles.taskMeta}>
            ...
            <View style={styles.metaItem}>
              <Icon name="event" size={20} color={colors.card_description} />
            ...
            <View style={styles.metaItem}>
              ...
              <Text style={[styles.taskMetaText, dynamicStyles.taskMetaText]}>{task.time}</Text>
            </View>
          </View>
            ...
            <View style={styles.subGoalsContainer}>
              {task.subGoals.map((sub, index) => (
                <View key={index} style={styles.subGoalRow}>
                  <Text style={[styles.subGoalText, dynamicStyles.subGoalText]}>{sub.name}</Text>
                  <TouchableOpacity style={styles.checkbox} onPress={() => toggleSubGoal(task.id, sub.subGoalId)}>
  ...
  return (
    <View style={[styles.container, dynamicStyles.container]}>
      {/* Header */}
      <View style={[styles.taskView, dynamicStyles.header]}>
      ...
      <View style={[styles.mainContainer, dynamicStyles.mainContainer]}>
        {loading ? (
          <View style={styles.loadingContainer}>
            ...
            <Text style={[styles.loadingText, { color: colors.text_alt }]}>Loading tasks...</Text>
          </View>
        ) : (
          <ScrollView
            style={{ flex: 1 }}
            ...
            <View style={styles.contentContainer}>
              {searchVisible && searchQuery.length > 0 ? (
                <View style={styles.searchResultsContainer}>
                  <Text style={[styles.sectionTitle, dynamicStyles.sectionTitle]}>Search Results</Text>
                  {Object.values(searchResults).flat().length > 0 ? (
                    Object.values(searchResults)
                      .flat()
                      .map((task, index) => <TaskCard key={`search-task-${task.id}-${index}`} task={task} />)
                  ) : (
                    <View style={styles.emptyTaskContainer}>
                      <Text style={[styles.emptyTaskText, dynamicStyles.emptyTaskText]}>No matching tasks found</Text>
                    </View>
                  )}
                </View>
              ) : (
                <>
                  <View style={[styles.calendarContainer, dynamicStyles.calendarContainer]}>
                    <View style={styles.calendarHeader}>
                      <TouchableOpacity onPress={() => changeMonth("prev")} style={styles.monthNavButton}>
                        <Icon name="chevron-left" size={24} color={colors.card_title} />
                      </TouchableOpacity>
                      ...
                      <TouchableOpacity onPress={() => changeMonth("next")} style={styles.monthNavButton}>
                        <Icon name="chevron-right" size={24} color={colors.card_title} />
                      </TouchableOpacity>
                    </View>
                  ...
                  {getTasksForSelectedDate.length > 0 ? (
                    ...
                    <View style={styles.emptyTaskContainer}>
                      <Text style={[styles.emptyTaskText, dynamicStyles.emptyTaskText]}>No tasks for this day</Text>
                    </View>
                  )}
                </>
              )}
      ...
      {/* FAB */}
      <TouchableOpacity onPress={goToTask} style={[styles.fab, dynamicStyles.fab]}>
